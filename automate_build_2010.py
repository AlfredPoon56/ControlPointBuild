...
version = "2010"					#Specifies the SharePoint version that this machine builds for
...
messagefile = '\\\\' + 'axstore\\development\\temp\\Buildmessage2010.txt'	#File on axstore to transmit messages to the mailing program
...
#Build the 5.1 Shell kit off the "trunk" in the nightly build.
call(["BuildVNextNative.bat", "Nightly", "2010"])
#Build the ControlPoint Online kit in the nightly build.
call(["BuildVNext.bat", "Nightly", "2010"])
call(["CopyNightlyBuildLog.bat", "2010", "T"])
...
with open("Build Log\Build_Native.txt") as log:
	for line in log:
		if line.count("errors,") > 0:			#Search for summary lines containing "errors,".
			if int(line.split()[3]) > 0:		#"line.split()[3] is the portion of line that contains the number of errors. Python's str.split() function splits on whitespace if no other separator is given.
				failed = 1
...
if failed:
	...
	message += "\n"
	message += "   \\\\Axstore\\Development\\Builds\\ControlPoint\\Nightly_Build_Logs\\2010\\Build_Native.txt "+native
	overall_failed = 1
else:
